...
const nlTranslations = {
  ...
  failedToLoad: "Kon de inhoud niet laden",
  loading: "Laden...",
};
...
type ContentProps = {
  ...
  language?: string; // Add language prop with default to 'nl'
};
...
export function Content({
  ...
  useEffect(() => {
    ...
    const loadMDXContent = async () => {
      try {
        // First try to load from the specific language directory (nl)
        try {
          ...
          // If not found in nl, continue to fallback logic
        }
        ...
        try {
          // If not found in ar or nl, try other language directories
          const languageDirs = [
            "en",
            "de",
            "es",
          ...
          setError(
            error instanceof Error
              ? error.message
              : "Onbekende fout bij het laden van inhoud",
          );
          setIsLoading(false);
        }
      }
  ...
  if (error || !module) {
    return (
      <div className="p-4 text-red-600">
        <h2 className="text-xl font-bold">{nlTranslations.errorLoading}</h2>
        <p>{error || nlTranslations.failedToLoad}</p>
      </div>
    );
  }
