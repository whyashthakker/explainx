...
export async function generateMetadata(props: {
  ...
  return {
    ...
    alternates: {
      canonical: `${baseUrl}/blog/post/zh-cn/${slug}`,
    ...
    openGraph: {
      type: "article",
      locale: "zh_CN",
      url: `${baseUrl}/blog/post/zh-cn/${slug}`,
      title: post.metadata.title,
      description: post.metadata.description,
      images: [
        {
...
export async function generateStaticParams(): Promise<{ slug: string }[]> {
  // Get MDX files from the Chinese Simplified directory
  const postsDirectory = path.join(process.cwd(), "app/blog/_posts/zh-cn");
...
export default async function Page(props: { params: Params }) {
  ...
  if (post.type === "database" && "content" in post) {
    ...
    // Note: Chinese text doesn't use spaces between words, so this estimation
    // is only approximate and likely undercounts actual word count
    wordCount = post.content.split(/\s+/).length;

    // Estimate reading time
    // Chinese reading speed might be different, but using similar estimate for consistency
