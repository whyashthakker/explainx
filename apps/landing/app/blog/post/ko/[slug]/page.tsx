...
export async function generateMetadata(props: {
  ...
  return {
    ...
    alternates: {
      canonical: `${baseUrl}/blog/post/ko/${slug}`,
...
export async function generateStaticParams(): Promise<{ slug: string }[]> {
  // Get MDX files from the Korean directory
  const postsDirectory = path.join(process.cwd(), "app/blog/_posts/ko");
...
export default async function Page(props: { params: Params }) {
  ...
  if (post.type === "database" && "content" in post) {
    ...
    // Note: Korean word counting differs from English due to lack of spaces
    // between some words. This is a simple approximation.
    wordCount = post.content.split(/\s+/).length;

    // Estimate reading time
    // Korean reading speed might be different, but using similar estimate for consistency
