...
const itTranslations = {
  ...
  failedToLoad: "Impossibile caricare il contenuto",
...
type ContentProps = {
  ...
  language?: string; // Add language prop with default to 'it'
};
...
export function Content({
  ...
  useEffect(() => {
    ...
    const loadMDXContent = async () => {
      try {
        // First try to load from the specific language directory (it)
        try {
          const mod = await import(`../../../_posts/it/${slug}.mdx`);
          setModule(mod as MDXModule);
          setIsLoading(false);
          return; // Exit if successful
        } catch (itError) {
          // If not found in it, continue to fallback logic
        }
        ...
        try {
          // If not found in ar or it, try other language directories
          const languageDirs = [
            "en",
            "de",
            "es",
          ...
          setError(
            error instanceof Error
              ? error.message
              : "Errore sconosciuto durante il caricamento del contenuto",
          );
          setIsLoading(false);
        }
      }
  ...
  if (error || !module) {
    return (
      <div className="p-4 text-red-600">
        <h2 className="text-xl font-bold">{itTranslations.errorLoading}</h2>
        <p>{error || itTranslations.failedToLoad}</p>
      </div>
    );
  }
