...
async function getPostFromMDX(
  ...
  try {
    ...
    if (!fs.existsSync(filePath)) {
      // Try to find the file in language-specific directories
      const langDirs = fs
        .readdirSync(path.join(process.cwd(), "app/blog/_posts"))
        .filter((dir) => dir !== "(en)" && !dir.includes("."));
      ...
      for (const langDir of langDirs) {
        ...
        if (fs.existsSync(langFilePath)) {
          ...
          return {
            ...
            metadata: {
              ...metadata,
              language: langDir === "(en)" ? "en" : langDir,
            },
          };
        }
      }
...
export async function generateMetadata(props: {
  ...
  return {
    ...
    openGraph: {
      type: "article",
      locale: post.metadata.language || "en",
    ...
    category: "technology",
    other: {
      "html-lang": post.metadata.language || "en",
    },
  };
}
...
export default async function Page(props: { params: Params }) {
  ...
  const defaultImage = ["../../../../public/images/blog/amazon-cosmo.jpeg"];
  ...
  return (
    <>
      <StructuredData
        ...
        description={post.metadata.description}
        mainEntityOfPage={`https://www.explainx.ai/blog/post/${slug}`}
        publisher={{
