...
type ContentProps = {
  ...
  language?: string; // Add language prop with default to 'zh-cn'
};
...
export function Content({
  ...
  useEffect(() => {
    ...
    const loadMDXContent = async () => {
      try {
        // First try to load from the specific language directory (zh-cn)
        try {
          const mod = await import(`../../../_posts/zh-cn/${slug}.mdx`);
          setModule(mod as MDXModule);
          setIsLoading(false);
          return; // Exit if successful
        } catch (zhCnError) {
          // If not found in zh-cn, continue to fallback logic
        }
        ...
        try {
          // If not found in ar or zh-cn, try other language directories
          const languageDirs = [
            "en",
            "de",
            "es",
          ...
          setError(
            error instanceof Error ? error.message : "加载内容时出现未知错误",
          );
          setIsLoading(false);
        }
      }
  ...
  if (error || !module) {
    return (
      <div className="p-4 text-red-600">
        <h2 className="text-xl font-bold">{zhCnTranslations.errorLoading}</h2>
        <p>{error || zhCnTranslations.failedToLoad}</p>
      </div>
    );
  }
