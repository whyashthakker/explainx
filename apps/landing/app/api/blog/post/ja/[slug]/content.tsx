...
const jaTranslations = {
  errorLoading: "コンテンツの読み込み中にエラーが発生しました",
  failedToLoad: "コンテンツを読み込めませんでした",
  loading: "読み込み中...",
};
...
type ContentProps = {
  ...
  language?: string; // Add language prop with default to 'ja'
};
...
export function Content({
  ...
  useEffect(() => {
    ...
    const loadMDXContent = async () => {
      try {
        // First try to load from the specific language directory (ja)
        try {
          const mod = await import(`../../../_posts/ja/${slug}.mdx`);
          setModule(mod as MDXModule);
          setIsLoading(false);
          return; // Exit if successful
        } catch (jaError) {
          // If not found in ja, continue to fallback logic
        }
        ...
        try {
          // If not found in ar or ja, try other language directories
          const languageDirs = [
            "en",
            "de",
            "es",
  ...
  if (error || !module) {
    return (
      <div className="p-4 text-red-600">
        <h2 className="text-xl font-bold">{jaTranslations.errorLoading}</h2>
        <p>{error || jaTranslations.failedToLoad}</p>
      </div>
    );
  }
