...
export function Content({ slug, metadata, dbContent }: ContentProps) {
  ...
  useEffect(() => {
    ...
    const loadMDXContent = async () => {
      try {
        // Try to load from the default (en) directory first
        const mod = await import(`../../_posts/(en)/${slug}.mdx`);
        setModule(mod as MDXModule);
        setIsLoading(false);
      } catch (enError) {
        try {
          // If not found in (en), try other language directories
          const languageDirs = [
            "ar",
            "de",
            "es",
