...
const hiTranslations = {
  ...
  failedToLoad: "सामग्री लोड करने में विफल",
...
type ContentProps = {
  ...
  language?: string; // Add language prop with default to 'hi'
};
...
export function Content({
  ...
  useEffect(() => {
    ...
    const loadMDXContent = async () => {
      try {
        // First try to load from the specific language directory (hi)
        try {
          const mod = await import(`../../../_posts/hi/${slug}.mdx`);
          setModule(mod as MDXModule);
          setIsLoading(false);
          return; // Exit if successful
        } catch (hiError) {
          // If not found in hi, continue to fallback logic
        }
        ...
        try {
          // If not found in ar or hi, try other language directories
          const languageDirs = [
            "en",
            "de",
            "es",
          ...
          setError(
            error instanceof Error
              ? error.message
              : "सामग्री लोड करने में अज्ञात त्रुटि",
          );
          setIsLoading(false);
        }
      }
  ...
  if (error || !module) {
    return (
      <div className="p-4 text-red-600">
        <h2 className="text-xl font-bold">{hiTranslations.errorLoading}</h2>
        <p>{error || hiTranslations.failedToLoad}</p>
      </div>
    );
  }
