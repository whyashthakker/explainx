...
const categories = [
  {
    ...
    keywords: ['influencer marketing', 'creator marketing', 'influencer', 'brand collaboration', 'brand partnership', 'sponsored content']
  },
  {
    id: 'creator-economy',
    name: 'Creator Economy',
    keywords: ['creator economy', 'content creator', 'digital creator', 'ugc creator', 'creator', 'influencer']
  },
  {
    id: 'social-media-marketing',
    name: 'Social Media Marketing',
    keywords: ['social media marketing', 'social media strategy', 'social marketing', 'digital marketing', 'social campaign']
  },
  {
    id: 'ai-tools',
    name: 'AI Tools',
    keywords: ['ai', 'artificial intelligence', 'machine learning', 'automation', 'ai-powered', 'ai matching']
  },
  {
    id: 'analytics',
    name: 'Analytics & ROI',
    keywords: ['analytics', 'roi', 'metrics', 'performance tracking', 'engagement rates', 'conversion rates', 'data-driven']
  },
  {
    id: 'instagram',
    name: 'Instagram',
    keywords: ['instagram', 'ig', 'reels', 'instagram stories', 'instagram posts']
  },
  {
    id: 'tiktok',
    name: 'TikTok',
  ...
  },
  {
    id: 'youtube',
    name: 'YouTube',
    keywords: ['youtube', 'youtube creator', 'youtube channel', 'video content']
  },
  {
    id: 'linkedin',
    name: 'LinkedIn',
    keywords: ['linkedin', 'professional networking', 'b2b influencer', 'business influencer']
  },
  {
    id: 'campaign-management',
    name: 'Campaign Management',
    keywords: ['campaign management', 'campaign tracking', 'campaign optimization', 'influencer campaign']
  },
  {
    id: 'brand-collaboration',
    name: 'Brand Collaboration',
    keywords: ['brand collaboration', 'brand partnership', 'sponsored content', 'brand deals', 'product seeding']
  },
  {
    id: 'verification',
    name: 'Creator Verification',
    keywords: ['verification', 'verified creator', 'authenticity check', 'quality assurance', 'fraud detection']
  }
];
...
function getContentString(content) {
  ...
  if (typeof content === 'object' && content !== null) {
    const relevantFields = ['title', 'description', 'content', 'excerpt', 'tags', 'categories']
...
function assignCategories(content) {
  ...
  if (matchedCategories.some(cat => platformCategories.includes(cat)) && 
  ...
  if ((matchedCategories.includes('brand-collaboration') || matchedCategories.includes('campaign-management')) && 
  ...
  if (matchedCategories.includes('ai-tools') && !matchedCategories.includes('analytics')) {
    matchedCategories.push('analytics');
  }

  return matchedCategories.length > 0 ? matchedCategories : ['influencer-marketing']; // Default to 'influencer-marketing'
}


function extractCompetitorIds(content) {
    try {
        const regex = /export\s+const\s+competitors\s*:\s*Record<string,\s*Competitor>\s*=\s*({[\s\S]*?});/;
...
const languages = ['en', 'es', 'de', 'fr', 'it', 'hi', 'ar', 'zh-cn', 'ko', 'ja', 'nl', 'ru', '(en)'];
...
function processProducts() {
  try {
    ...
    const productDataPath = path.join(process.cwd(), 'apps/landing/data/productPageData.ts');
    ...
    console.error('Attempted to read from:', path.join(process.cwd(), 'apps/landing/data/productPageData.ts'));
    return [];
  }
}
...
function processCompetitors() {
  try {
    const competitorsPath = path.join(process.cwd(), 'apps/landing/data/competitors.ts');
    ...
    console.error('Attempted to read from:', path.join(process.cwd(), 'apps/landing/data/competitors.ts'));
    return [];
  }
}
...
function extractUseCaseData(content) {
  try {
    ...
    const regex = /export\s+const\s+useCases\s*:\s*UseCase\[\]\s*=\s*(\[[\s\S]*?\];)/;
    const match = content.match(regex);
...
function processUseCases() {
  try {
    const useCasesPath = path.join(process.cwd(), 'apps/landing/data/use-cases/data.ts');
...
`;
    ...
    fs.writeFileSync(path.join(dataDir, 'use-case-data.ts'), useCaseDataContent);
...
function processTools() {
  try {
    const toolDataPath = path.join(process.cwd(), 'apps/landing/data/tool-data.ts');
...
function processAllPosts() {
  ...
  function processLanguagePosts(lang, subdir) {
    ...
    const files = fs.readdirSync(langPostsDirectory)
...
function generateProductData(productPageData) {
  try {
    ...
    const productObjectRegex = /{[\s\S]*?slug:\s*"([^"]+)"[\s\S]*?metadata:\s*{[\s\S]*?title:\s*"([^"]+)"[\s\S]*?description:\s*"([^"]+)"[\s\S]*?}/g;
    const products = [];
    let productMatch;
...
try {
  ...
  console.log(`Processed ${tools.length} tools:`, tools.map(t => t.url).join(', '));
  
  const products = processProducts();
  console.log(`Processed ${products.length} products:`, products.map(p => p.url).join(', '));
  
  const competitors = processCompetitors();
  console.log(`Processed ${competitors.length} competitor pages:`, competitors.map(c => c.url).join(', '));

  const useCases = processUseCases();
  console.log(`Processed ${useCases.length} use cases:`, useCases.map(u => u.url).join('\n'));
  ...
  const comparisonSitemapContent = generateSitemapContent(competitors, 'comparison');
  ...
  fs.writeFileSync(path.join(outputDir, 'blog-sitemap.xml'), blogSitemapContent);
  fs.writeFileSync(path.join(outputDir, 'product-sitemap.xml'), productSitemapContent);
  fs.writeFileSync(path.join(outputDir, 'comparison-sitemap.xml'), comparisonSitemapContent);
  fs.writeFileSync(path.join(outputDir, 'use-case-sitemap.xml'), useCaseSitemapContent);
  fs.writeFileSync(path.join(outputDir, 'tools-sitemap.xml'), toolsSitemapContent);
